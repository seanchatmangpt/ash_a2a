...
defmodule AshA2A.ArchitectureVerifier.ChicagoRollupTest do
  ...
  ash_a2a.verify_architecture` previously ran only the 9 checks in
  `test/ash_a2a_architecture_verifier_test.exs` and rolled up none of the
  ...
  @tag timeout: 600_000
  test "ArchitectureVerifier.checks/0 rolls up all seven real Chicago courts alongside the original nine" do
    results = ArchitectureVerifier.checks()

    # 9 original checks + 7 Chicago-court rollup checks, additive per the
    # brief -- the original 243-line test file's own "9 checks" assertion
