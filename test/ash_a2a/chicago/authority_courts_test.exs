defmodule AshA2A.Chicago.AuthorityCourtsTest do
  ...
  describe "declarations and discovery" do
    ...
    test "the harness admits one mapping per authority activity" do
      mappings = AuthorityHarness.mappings()
      ...
      # The shared set is attributed to the harness that defines its closures,
      # so the runner admits it once however many authority courts are selected.
      assert Enum.all?(mappings, &(&1.source == AuthorityHarness))

      admitted = Runner.ocel_mappings([AuthorityNonImplication, GrantLifecycle])

      for m <- mappings do
        assert Enum.count(admitted, &(&1.event == m.event and &1.activity == m.activity)) == 1,
               m.activity
      end
    end
  end
