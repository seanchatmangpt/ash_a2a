defmodule AshA2A.Chicago.MutationHarnessTest do
  ...
  describe "engine: build, load, always restore" do
    test "every §97 catalog target is a real function in this subject or blocked with a reason" do
      ...
      # RFC-SA2A-001 S20 root manifest is not compiled in this subject.
      assert %{status: :blocked, code: :mutation_target_not_compiled} =
               resolution["root_manifest_component_drift"].target_status
    ...
    test "a private function can be mutated (CommandBus.prepare_receipt_anchor/3)" do
      mutation = Catalog.fetch!("remove_receipt_preparation")
      assert {:ok, plan} = Mutation.prepare(mutation)
      assert plan.clauses_mutated == 2
      refute function_exported?(CommandBus, :prepare_receipt_anchor, 3)
    ...
    test "refusals are typed and nothing is loaded" do
      ...
      assert {:error, %{code: :mutation_target_not_compiled}} =
               Mutation.prepare(%{base | module: AshA2A.Semantic.RootManifest})
  ...
  describe "SA2A-MUTATION court end to end" do
    test "every falsifier's verdict, corroborated by the independent OCEL consumer", %{
      ...
      expected_kills = %{
        ...
        "remove_receipt_preparation" => ~w(CHI-MUTGUARD-005 CHI-MUTGUARD-008),
      ...
      assert resolution["root_manifest_component_drift"].target_status.status == :blocked
