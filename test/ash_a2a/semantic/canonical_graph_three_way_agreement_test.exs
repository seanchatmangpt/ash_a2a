defmodule AshA2A.Semantic.CanonicalGraphThreeWayAgreementTest do
  ...
  **Graphs containing blank nodes: the two engines DIVERGE**, and the
  ...
  process. It does not stub, fake or simulate graphlaw. When the vendored
  artifact is not present on the current branch -- it arrives with
  `feat/graphlaw-vendor-pipeline-v26.9.16`, which this corrective branch does
  not include -- the test SKIPS visibly, naming what is missing. It never
  substitutes a double for the engine under comparison.
  """
  ...
  if @missing != [] do
    @moduletag skip:
                 "graphlaw wasm law package not available on this branch; missing: " <>
                   ...
                   " (arrives with feat/graphlaw-vendor-pipeline-v26.9.16)"
  end
