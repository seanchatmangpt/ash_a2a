defmodule AshA2A.GraphLawWasmTest do
  @moduledoc """
  Chicago-school tests for `AshA2A.GraphLaw.Wasm`.
  ...
  setup_all do
    ...
    {:ok, pid} = start_supervised({Wasm, name: :graphlaw_wasm_test})
    %{engine: pid}
  end
  ...
  describe "artifact vendoring" do
    ...
    test "MANIFEST.json records the artifact's REAL on-disk digests" do
      assert {:ok, manifest} = Wasm.manifest()

      real_bytes = File.read!(Wasm.wasm_path())
      real_sha256 = real_bytes |> then(&:crypto.hash(:sha256, &1)) |> Base.encode16(case: :lower)

      assert manifest["sha256"] == real_sha256,
             "MANIFEST.json sha256 does not match the bytes actually on disk"

      assert manifest["size_bytes"] == byte_size(real_bytes)
    end

    test "MANIFEST.json's recorded blake3 is re-derivable by a real external BLAKE3 tool" do
      ...
      case System.find_executable("b3sum") do
        nil ->
          IO.puts(
            "\n[SKIPPED ASSERTION] b3sum not on PATH; MANIFEST.json blake3 " <>
              "re-derivation not performed on this machine."
          )

        b3sum ->
          {out, 0} = System.cmd(b3sum, [Wasm.wasm_path()])
          [measured | _] = String.split(out, ~r/\s+/, trim: true)
          assert measured == manifest["blake3"]
      end
    end

    test "MANIFEST.json's recorded engine version is what the REAL engine reports" do
      assert {:ok, manifest} = Wasm.manifest()
      assert {:ok, reported} = Wasm.version(:graphlaw_wasm_test)
      ...
      case System.find_executable("b3sum") do
        ...
        b3sum ->
          ...
          try do
            ...
            assert {:ok, ^external} = Wasm.blake3_hex(content, :graphlaw_wasm_test)
          after
            File.rm(path)
          end
      end
  ...
  describe "version/1" do
    test "returns the engine's REAL self-reported version", %{engine: _} do
      assert {:ok, version} = Wasm.version(:graphlaw_wasm_test)
  ...
  describe "graph_hash/3 -- RFC S12 canonical graph identity" do
    ...
    test "is INVARIANT under prefix relabelling and triple reordering" do
      assert {:ok, base} = Wasm.graph_hash(@base_ttl, :graphlaw_wasm_test)
      assert {:ok, reordered} = Wasm.graph_hash(@reordered_ttl, :graphlaw_wasm_test)
    ...
    test "DISTINGUISHES a graph whose content actually changed" do
      assert {:ok, changed} = Wasm.graph_hash(@changed_ttl, :graphlaw_wasm_test)
    ...
    test "is stable across repeated real calls on one reused instance" do
      digests =
        for _ <- 1..10 do
          {:ok, d} = Wasm.graph_hash(@base_ttl, :graphlaw_wasm_test)
          d
        end
    ...
    test "round-trips non-ASCII literals through linear memory correctly" do
      ...
      assert {:ok, digest} = Wasm.graph_hash(unicode, :graphlaw_wasm_test)
  ...
  describe "blake3_hex/3" do
    ...
    test "matches the published BLAKE3 digest of the empty input" do
      assert {:ok, @blake3_empty} = Wasm.blake3_hex("", :graphlaw_wasm_test)
    end
  end
  ...
  describe "validate_all/7" do
    ...
    test "ADMITS a graph that satisfies real SHACL shapes" do
      assert {:ok, result} =
               Wasm.validate_all(@base_ttl, "", @shacl_shapes, "", "", :graphlaw_wasm_test)
    ...
    test "REFUSES an unparseable SHACL shapes graph, with a real parser diagnostic" do
      assert {:ok, result} =
               Wasm.validate_all(@base_ttl, "", "!!!not shacl!!!", "", "", :graphlaw_wasm_test)
  ...
  describe "malformed input does not crash or poison the instance" do
    test "graph_hash/3 on garbage Turtle returns a value, and the instance still works" do
      ...
      assert {:ok, garbage_digest} =
               Wasm.graph_hash("this is not turtle @@@ <<<", :graphlaw_wasm_test)
    ...
    test "a long sequence of mixed good/bad inputs leaves the instance correct" do
      ...
      results =
        Enum.map(inputs, fn ttl ->
          assert {:ok, digest} = Wasm.graph_hash(ttl, :graphlaw_wasm_test)
          digest
        end)
      ...
      results =
        ...
        |> Task.async_stream(
          fn i ->
            ...
            {:ok, digest} = Wasm.graph_hash(ttl, :graphlaw_wasm_test)
            digest == expected
          end,
          max_concurrency: 20,
          timeout: 60_000
  ...
  describe "graceful degradation when the artifact is absent" do
    test "starts anyway and returns a typed error on every call" do
      ...
      {:ok, _pid} =
        start_supervised(
          {Wasm, name: :graphlaw_wasm_absent_test, wasm_path: missing},
          id: :graphlaw_absent
        )
