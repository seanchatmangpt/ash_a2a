defmodule AshA2A.ReceiptReplayCountingActuatorTest do
  ...
  test "a real external_do executes exactly once, and replaying its receipt never re-actuates", %{
    ...
    assert ActuationCounter.count(effect_key) == 0
    ...
    real_count_after_execution = ActuationCounter.count(effect_key)
    ...
    assert ActuationCounter.count(effect_key) == real_count_after_execution
    assert ActuationCounter.count(effect_key) == 1
  end
