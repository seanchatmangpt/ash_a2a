defmodule AshA2A.SemanticCrossPeerTest do
  ...
  defp peer_a_message(graph, opts \\ []) do
    envelope =
      Envelope.new!(
        [origin_peer: @peer_a, graph: graph] ++
          Keyword.take(opts, [:graph_digest, :consequence_class, :capability_iri])
      )
  ...
  describe "Message_A -> Candidate_B -> GraphLaw_B -> O*_B" do
    ...
    test "there is NO transition edge from :received to :admitted, at all" do
      assert {:error, :illegal_standing_transition} =
               Standing.transition(:received, :admitted)

      refute :admitted in Standing.successors(:received)
      assert Standing.successors(:received) == [:candidate, :unsupported, :refused]
    end
    ...
    test "peer A's claim of admitted standing is recorded and NOT honored", %{
      plug_opts: plug_opts,
      ledger: ledger
    } do
      ...
      # The graph itself conforms, so peer B admits it -- on peer B's own
      # evidence, and only after passing through :candidate.
      assert data["standing"] == "admitted"
      assert data["over_claimed"] == true
      assert Ledger.path(ledger, envelope.envelope_id) == [:received, :candidate, :admitted]

      # And the parsed envelope's standing was never the claimed value.
      {:ok, parsed} = Envelope.parse(forged_message)
      assert parsed.standing == :candidate
      assert parsed.claimed_standing == "admitted"
      assert parsed.claimed_authority == "root"
      assert Envelope.over_claimed?(parsed)
    end
  ...
  describe "S75 -- traffic from a non-Semantic A2A peer inherits NO standing" do
    ...
    test "an envelope declaring a DIFFERENT profile is refused, not downgraded", %{
      ...
      wrong_profile =
        envelope |> Envelope.to_payload() |> Map.put("profile", "SA2A-PROFILE-v99.0.0")
  ...
  describe "S50 -- an A2A Artifact gains no standing from being an Artifact" do
    ...
    test "an envelope carried inside an artifact parses as a CANDIDATE, not admitted" do
        ...
        A2A.Artifact.new([A2A.Part.Text.new("payload")],
          metadata: %{
            Extension.extension_key() =>
              envelope |> Envelope.to_payload() |> Map.put("standing", "admitted")
          }
        )

      assert {:ok, parsed} = Peer.envelope_from_artifact(artifact)
      assert parsed.standing == :candidate
      assert parsed.claimed_standing == "admitted"
    end
  ...
  describe "fail-closed" do
    test "a peer whose engine is genuinely unreachable returns :unsupported, never :admitted" do
      ...
      try do
        ...
        envelope = Envelope.new!(origin_peer: @peer_a, graph: Graphs.conforming_order())
    ...
    test ":admitted and :refused are terminal; :unsupported is not admitted" do
