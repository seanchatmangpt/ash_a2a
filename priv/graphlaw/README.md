...
non-BEAM), driven from Elixir by `AshA2A.GraphLaw.WasmtimeRuntime`.
...
`AshA2A.GraphLaw.WasmtimeRuntime` independently hashes the file it pointed the host at
...
from, so two runtimes cannot drift apart by each keeping their own
...
- `test/ash_a2a/graph_law/wasmtime_runtime_test.exs` — real conformance vectors
- `MANIFEST.json` / `WASMEX_HOST_MANIFEST.json` — the content-addressed artifact records
