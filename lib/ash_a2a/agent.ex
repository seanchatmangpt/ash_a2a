defmodule AshA2A.Agent do
  ...
  *same* agent instance until it completes. This is inherent to `A2A.Agent`'s
  ...
  # yield that string-keyed shape, so the atom-keyed shape is the one a remote
  # caller cannot forge. Accepting a string-keyed `%{"identity" => _}` let a
  # caller with a valid bearer token rebind it to another principal (an
  # authenticated, ungranted caller impersonating a granted principal and
