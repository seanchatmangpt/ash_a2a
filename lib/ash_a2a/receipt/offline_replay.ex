defmodule AshA2A.Receipt.OfflineReplay do
  ...
  defp basis(command_id, prepared, final) do
    for %Receipt{} = receipt <- [prepared, final],
      ...
      fail(
        :replay_basis_refused,
        command_id,
        "#{receipt.status} receipt: #{code}: #{format_detail(detail)}"
      )
    end
  end

  # `AshA2A.Receipt.Binding.check/2` (RFC-SA2A-002 Gate 9) refuses with a
  # `detail` that is sometimes a plain string and sometimes a term (e.g.
  # `%{fields: [...]}` from a field mismatch, `%{bound: ..., configured: ...}`
  # from a key mismatch) -- `Replay.basis/1` passes that term through
  # unchanged. Never assume `detail` is a binary just because it often is.
