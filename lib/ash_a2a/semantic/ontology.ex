defmodule AshA2A.Semantic.Ontology do
  ...
  ### `canonical_digest/1` -- engine graph digest, portable across peers

  `canonical_digest/1` serialises the triples to N-Triples and hands them to
  the real `praxis-graphlaw` engine via `AshA2A.GraphLaw.WasmDriver.graph_hash/2`,
  which normalizes prefixes and triple order before hashing. This is the
  digest two peers running the same wasm bytes can compare, and the same
  engine digest `AshA2A.Semantic.AdmissionHash` consumes. It requires the
  engine to be present and returns a typed `{:error, map}` when it is not.

  Measured limitation: the engine digest is **not** blank-node-relabel
  invariant, so it is not RDFC-1.0 (see `AshA2A.GraphLaw.Wasm.graph_hash/2`).
  `AshA2A.Semantic.CanonicalDigest.canonical_digest/2` is the sibling path over
  the same engine that adds `AshA2A.Semantic.Serialize`'s parse-back gate and
  reports triple counts; the two serialise objects differently, so their
  digests of the same ontology are not expected to be equal.
  ...
  `AshA2A.GraphLaw.WasmDriver.graph_hash/2`, so the returned digest is invariant
  ...
  def canonical_digest(triples, opts) when is_list(triples),
    do: triples |> to_ntriples() |> WasmDriver.graph_hash(opts)
  ...
  rendered lines and sorting bytes is not graph canonicalization, and the
  byte sort here is a plain `Enum.sort/1`, not an RDF canonical ordering.
  This writer emits no blank nodes (every subject and IRI-shaped object is
  written as `<iri>`), so the engine digest's measured blank-node-relabel
  sensitivity cannot arise from `to_ntriples/1` output.
  """
