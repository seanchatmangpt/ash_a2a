defmodule AshA2A.Semantic.AdmissionHash do
  ...
  from `AshA2A.GraphLaw.Wasm`, i.e. from the real `praxis-graphlaw` wasm
  ...
  **2. Domain separation.** Each component's preimage is prefixed with its
  ...
  Component digests:
      ...
      graph_digest      = GraphLaw.Wasm.graph_hash(graph_ttl)     # RDFC-1.0-family canonical
      validators_digest = GraphLaw.Wasm.blake3_hex(validators_preimage)
      rules_digest      = GraphLaw.Wasm.blake3_hex(rules_preimage)
      profile_digest    = GraphLaw.Wasm.blake3_hex(profile_preimage)
  ...
  Composite preimage and the hash itself:
      ...
      admission_hash = GraphLaw.Wasm.blake3_hex(composite)
  ...
  Delegates to the real engine -- `AshA2A.GraphLaw.Wasm.graph_hash/2` -- so
  the digest is a genuine RDFC-1.0-family canonical-form hash and not an
  Elixir-side sort-then-hash.
  """
  ...
  def canonical_graph_digest(graph_ttl, opts \\ []) when is_binary(graph_ttl),
    do: Wasm.graph_hash(graph_ttl, opts)
  ...
  Options:
    ...
    * `:wasm_path`, `:tmp_dir` -- forwarded to `AshA2A.GraphLaw.Wasm`.
  ...
  def admission_hash(graph_ttl, validators, rules, profile, opts \\ [])
    ...
    with {:ok, results} <- Wasm.call_many(calls, opts),
         ...
         {:ok, hash} <- Wasm.blake3_hex(composite, opts) do
      {:ok,
       %__MODULE__{
         admission_hash: hash,
         graph_digest: graph,
