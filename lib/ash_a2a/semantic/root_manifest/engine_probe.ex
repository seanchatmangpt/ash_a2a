defmodule AshA2A.Semantic.RootManifest.EngineProbe do
  ...
  `priv/graphlaw/graphlaw_host.mjs` is the committed host: it instantiates
  the same `.wasm` bytes manually against the wasm-bindgen ABI and prints one
  JSON object. It adds no semantics.
  ...
  @host_relative "graphlaw/graphlaw_host.mjs"
  ...
  def host_path(opts \\ []) do
    Keyword.get(opts, :host_path) ||
      Application.get_env(:ash_a2a, :graphlaw_host_path) ||
