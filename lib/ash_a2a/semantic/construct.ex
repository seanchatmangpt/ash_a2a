...
defmodule AshA2A.Semantic.Construct do
  ...
  `AshA2A.Semantic.CanonicalDigest.digest/1` emits exactly that shape, so
  ...
  *output* has its own separate `artifact_digest`. Digesting the artifact
  ...
  alias AshA2A.Semantic.{CanonicalDigest, Construction, PlanPackage, PlanProjection}
  ...
  *and* re-run the S27 checks (`PlanProjection.verify_self/1`) at
  ...
  def construct(%PlanPackage{} = package, %PlanProjection{} = projection, manufacturer, opts)
    ...
    with :ok <- verify_projection(projection),
      ...
      artifact_digest = CanonicalDigest.digest(artifact)
  ...
  anything that is neither spelling. `AshA2A.Semantic.CanonicalDigest.digest/1`
  ...
  defp construction_receipt(
    ...
    Map.put(body, :receipt_digest, CanonicalDigest.digest(body))
  end
