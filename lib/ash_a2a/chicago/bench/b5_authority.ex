defmodule AshA2A.Chicago.Bench.B5Authority do
  ...
  | §89 measure                         | phase                          |
  |-------------------------------------|--------------------------------|
  | authority decision latency          | `authority_decision` (t0 -> `authority.grant`) |
  ...
  defp invariant("authorized", reply, timeline, present?) do
    checks = [
      ...
      {outcome_of(timeline, @grant) == :granted,
       "authority.grant outcome #{inspect(outcome_of(timeline, @grant))}"},
