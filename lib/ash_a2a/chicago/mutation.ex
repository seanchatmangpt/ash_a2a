defmodule AshA2A.Chicago.Mutation do
  ...
  defp judge_court(id, {:ok, baseline}, {:ok, mutant}) do
    ...
    cond do
      b.failed != [] -> {:unknown, "baseline already failing: #{Enum.join(b.failed, ", ")}"}
      b.passed == [] -> {:unknown, "baseline has no corroborated pass"}
      new_failures != [] -> {:killed, new_failures}
      new_unresolved == [] and mu.passed != [] -> :survived
      true -> {:unknown, "mutant run unresolved: #{Enum.join(new_unresolved, ", ")}"}
    end
  end
