defmodule AshA2A.Chicago.Courts.Benchmarks do
  ...
  def ocel_mappings do
    [
      Mapping.new!(
        event: [:ash_a2a, :authority, :grant],
        activity: "authority.grant",
        source: __MODULE__,
        objects: fn _m, meta ->
          [
        ...
        attributes: fn _m, meta -> Map.take(meta, [:outcome, :code, :policy]) end
      )
    ]
  end
  ...
  def falsifiers do
    [
      ...
      Falsifier.new!(
        ...
        attempt_evidence:
          "authority.grant granted AND refused, brce.admission admitted AND refused(authority_required), brce.prepare prepared, brce.actuate.stop ok, brce.commit committed attributed to this stimulus",
        survival_evidence:
        ...
        attempt_predicate:
          {:all,
           [
             {:observed, "authority.grant", %{"outcome" => "granted"}},
             {:observed, "authority.grant", %{"outcome" => "refused"}},
  ...
  def run(%Context{} = ctx) do
    ...
    [
      ...
      measure(ctx, b5, B5Authority, opts, record_opts, ["authority.grant", "brce.commit"]),
