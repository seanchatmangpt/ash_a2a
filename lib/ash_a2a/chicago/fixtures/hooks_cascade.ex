...
defmodule AshA2A.Chicago.Fixtures.HooksCascade do
  ...
  The source is this fixture module -- it defines the closures -- so the
  identical set declared by both `SA2A-HOOK` and `SA2A-CASCADE` is admitted
  once by `AshA2A.Chicago.Runner.ocel_mappings/1` (one emission, one record;
  count predicates never double count), the mapping digest versions the code
  that interprets the events, and the observer non-authority proof reaches
  the closures from `mappings/0`.
  """
