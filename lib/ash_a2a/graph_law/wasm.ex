defmodule AshA2A.GraphLaw.Wasm do
  ...
  This is the *only* place in `ash_a2a` that computes canonical RDF graph
  identity or BLAKE3 digests. Elixir deliberately owns none of that
  semantics: the engine (`praxis-graphlaw`, native N3/Datalog/SPARQL/SHACL/
  ShEx, built on `oxrdf` with the `rdfc-10` feature) owns canonicalization,
  and both sides of a portable-conformance comparison must therefore execute
  the *same* wasm bytes. Elixir's job is the envelope, the standing, the
  refusal typing and the receipt -- never the graph algebra.
  ...
  undecodable stdout).
      ...
      iex> {:ok, [{:ok, v}]} = AshA2A.GraphLaw.Wasm.call_many([{"graphlaw_version", []}])
  ...
  This is real canonicalization (the engine builds on `oxrdf`'s `rdfc-10`
  feature), not a sort-then-hash: relabelling prefixes and reordering
  triples do not change the result, while changing a single triple does.
  """
