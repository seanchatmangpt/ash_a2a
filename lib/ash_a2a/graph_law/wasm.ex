defmodule AshA2A.GraphLaw.Wasm do
  ...
  WebAssembly artifact, vendored at
  `priv/graphlaw/praxis_graphlaw_wasm.wasm` and recorded in
  ...
  **GraphLaw derives and validates; it never authorizes and never
  ...
  `test/ash_a2a/graph_law_wasm_test.exs`, which asserts digests measured
  ...
  `config :ash_a2a, :graphlaw_wasm_path`, else
  `priv/graphlaw/praxis_graphlaw_wasm.wasm` inside this application's real
  `priv` directory.
  ...
  defp default_wasm_path do
    case :code.priv_dir(:ash_a2a) do
      {:error, :bad_name} ->
        Path.expand("../../../priv/graphlaw/praxis_graphlaw_wasm.wasm", __DIR__)

      priv ->
        Path.join([to_string(priv), "graphlaw", "praxis_graphlaw_wasm.wasm"])
    end
  end

  @doc """
  Reads the real `priv/graphlaw/MANIFEST.json` artifact record.

  This is the honest artifact record that the RFC S21 Root Manifest builds
  on (that module is owned elsewhere); this function only surfaces what was
  actually measured about the bytes on disk.
  """
  @spec manifest(keyword()) :: {:ok, map()} | {:error, error()}
  def manifest(opts \\ []) do
    path =
      Keyword.get(opts, :manifest_path) ||
        Path.join(Path.dirname(wasm_path(opts)), "MANIFEST.json")
  ...
  def init(opts) do
    ...
    case load(path) do
      ...
      {:error, reason} ->
        Logger.warning(
          "AshA2A.GraphLaw.Wasm: engine unavailable (#{inspect(reason)}). " <>
