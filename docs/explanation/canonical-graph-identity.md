...
repository should compute a canonical graph identity.
...
`AshA2A.GraphLaw.Wasm.graph_hash/3` (on `feat/graphlaw-wasmex-bridge-v26.9.16`) guards only
`is_binary/1`. The wasm export it calls takes a `&str`. A single `0xFF` byte is therefore
not a document the engine rejects but a marshalling failure: the verifier measured the
engine committing 2,148,270,080 bytes and permanently poisoning the instance.
...
"Required fix for the wasm bridge branch" below.
...
## Required fix for the wasm bridge branch

`feat/graphlaw-wasmex-bridge-v26.9.16` needs this applied at merge. In
`lib/ash_a2a/graph_law/wasm.ex`, `graph_hash/3` and `blake3_hex/3` currently guard only
`is_binary/1`:
...
Every export taking a Rust `&str` must reject non-UTF-8 input before `transact/4`:

```elixir
def graph_hash(ttl, server \\ __MODULE__, timeout \\ @default_timeout) when is_binary(ttl) do
  with :ok <- AshA2A.Semantic.CanonicalGraph.ensure_utf8(ttl),
...
`ensure_utf8/1` returns `:ok` or `{:error, {:invalid_encoding, byte_offset}}`, so the `with`
short-circuits into the existing error channel and `{:invalid_encoding, offset}` needs
adding to that module's `t:error/0`. `blake3_hex/3` takes the same treatment, and so does
any future export whose Rust signature takes `&str` rather than `&[u8]`.

Two things this branch deliberately did not do, because they are merge-time decisions rather
than corrections:

- It did not edit the bridge or vendor branches. This is a fresh corrective branch from
  `main`, so the guard lives in `CanonicalGraph` where it is testable today, and the bridge
  edit is specified here for application at merge.
- It did not vendor the 3.2MB wasm artifact into this branch to make the three-way test run
  unconditionally. Duplicating it would collide with the vendor branch that owns it; the
  test is written to find it at that branch's own paths and light up automatically once the
  two are merged.
...
- `docs/explanation/graphlaw-wasm-integration.md` -- the vendoring pipeline (arrives with
  `feat/graphlaw-vendor-pipeline-v26.9.16`)
